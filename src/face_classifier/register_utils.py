    ...
    if(oheight >= owidth):
        ...
        if(((112 - resized.shape[1]) / 2) != 0):
            resized = np.column_stack(
                (np.zeros((resized.shape[0], 1, 3), np.uint8), resized))
        return resized
        ...
        if(((112 - resized.shape[0]) / 2) != 0):
            ...
            resized = np.row_stack(
                (np.zeros((1, resized.shape[1], 3), np.uint8), resized))
        return resized
...
def update_model_face(recognizer):
    path = "dataset/"
    ...
    for folder in os.listdir(path):
        ...
        if faces:
            for face in faces:
                if face is not None:
                    embeddings = np.row_stack(
                        (embeddings, recognizer.get_embedding(face)))
                    names.append(folder)
